...
-- (9) 2. PowerShell:
--     docker compose exec db bash
--         mysql --user=root --password=p < /sql/create-db-arzt.sql
--         exit
--     docker compose down
...
CREATE USER IF NOT EXISTS arzt IDENTIFIED BY 'p';
GRANT USAGE ON *.* TO arzt;

-- https://dev.mysql.com/doc/refman/9.2/en/create-database.html
-- https://dev.mysql.com/doc/refman/9.2/en/charset.html
-- SHOW CHARACTER SET;
CREATE DATABASE IF NOT EXISTS arzt CHARACTER SET utf8;

GRANT ALL PRIVILEGES ON arzt.* to arzt;

-- https://dev.mysql.com/doc/refman/9.2/en/create-tablespace.html
-- .idb-Datei innerhalb vom "data"-Verzeichnis
CREATE TABLESPACE `arztspace` ADD DATAFILE 'arztspace.ibd' ENGINE=INNODB;
